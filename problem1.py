def add_map(fun, L):
    ...
    """
    # Fill in
    #list(map(lambda x: fun(x) + 1, L))  #How do I test this
    return [fun(i) + 1 for i in L] #In class implementation for using the function within a function
...
def compose_map(fun1, fun2, L):
    ...
    """
    # Fill in
    return list(fun2(fun1(i)) for i in L) #use the lsit operator to call the func being called within a func to return a list
...
def compose(fun1, fun2):
    ...
    def ret_fun(i):
        # Fill in
        return fun1(fun2(i)) #return the fun1 calling fun2 with i as an input 
...
def repeater(fun, num_repeats):
    ...
    def ret_fun(x):
        # Fill in
        for i in range(num_repeats): #Cannot do just if statemetns cuz it should apply to all numbers of num_repeats, must use the for loop
            x = fun(x)
        return x
